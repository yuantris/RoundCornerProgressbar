...
open class IconRoundCornerProgressBar : AnimatedRoundCornerProgressBar {
    ...
    override fun initStyleable(context: Context, attrs: AttributeSet?) {
        ...
        with(typedArray) {
            iconResource = getResourceId(R.styleable.IconRoundCornerProgressBar_fyIconSrc, -1)
            iconSize = getDimension(R.styleable.IconRoundCornerProgressBar_fyIconSize, -1f).toInt()
            iconWidth = getDimension(
                R.styleable.IconRoundCornerProgressBar_fyIconWidth,
                dp2px(DEFAULT_ICON_SIZE.toFloat())
            ).toInt()
            iconHeight = getDimension(
                R.styleable.IconRoundCornerProgressBar_fyIconHeight,
                dp2px(DEFAULT_ICON_SIZE.toFloat())
            ).toInt()
            iconPadding =
                getDimension(R.styleable.IconRoundCornerProgressBar_fyIconPadding, -1f).toInt()
            iconPaddingLeft = getDimension(
                R.styleable.IconRoundCornerProgressBar_fyIconPaddingLeft,
                dp2px(DEFAULT_ICON_PADDING_LEFT.toFloat())
            ).toInt()
            iconPaddingRight = getDimension(
                R.styleable.IconRoundCornerProgressBar_fyIconPaddingRight,
                dp2px(DEFAULT_ICON_PADDING_RIGHT.toFloat())
            ).toInt()
            iconPaddingTop = getDimension(
                R.styleable.IconRoundCornerProgressBar_fyIconPaddingTop,
                dp2px(DEFAULT_ICON_PADDING_TOP.toFloat())
            ).toInt()
            iconPaddingBottom = getDimension(
                R.styleable.IconRoundCornerProgressBar_fyIconPaddingBottom,
            ...
            colorIconBackground = getColor(
                R.styleable.IconRoundCornerProgressBar_fyIconBackgroundColor,
